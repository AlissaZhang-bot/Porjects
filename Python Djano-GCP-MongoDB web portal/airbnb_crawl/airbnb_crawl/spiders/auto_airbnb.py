...
from airbnb_crawl.items import AirbnbCrawlItem

# *********************************************************************************************
# Run crawler with -> scrapy crawl auto_airbnb -o crawl.json -a city=''                            *
# *********************************************************************************************
...
class AirbnbSpider(scrapy.Spider):
    ...
    def start_requests(self):
        '''Sends a scrapy request to the designated city'''

        url = ('https://www.airbnb.com.au/api/v2/explore_tabs?_format=for_explore_search_web&'
            'auto_ib=false&client_session_id=cccfa020-6e1a-4b00-af66-7ac61d752ab7&'
            'currency=AUD&current_tab_id=home_tab&experiences_per_grid=20&fetch_filters=true&'
            'guidebooks_per_grid=20&has_zero_guest_treatment=true&hide_dates_and_guests_filters=true'
            '&is_guided_search=true&is_new_cards_experiment=true&is_standard_search=true&'
            'items_per_grid=18&key=d306zoyjsyarp7ifhu67rjxn52tv0t20&locale=en-AU'
            '&metadata_only=false&query={0}&query_understanding_enabled=true&refinement_paths%5B%5D=%2Fhomes'
            '&satori_parameters=ERIA&satori_version=1.1.9&screen_height=722&screen_size=medium&screen_width=810'
            '&selected_tab_id=home_tab&show_groupings=true&supports_for_you_v3=true&timezone_offset=660'
            '&version=1.6.2')
        # form url using parameters
        new_url = url.format(self.city)
        # define a callback function to retrieve data
    ...
    def parse_id(self, response):
        ...
        # base_url is to form a new url with retrieved property ids
        ...
        for home in homes:
            ...
            data_dict[room_id]['picture_urls'] = home.get('listing').get('picture_urls')
        ...
        if pagination_metadata.get('has_next_page'):
            ...
            new_url = ('https://www.airbnb.com.au/api/v2/explore_tabs?_format=for_explore_search_web&'
            'auto_ib=false&client_session_id=cccfa020-6e1a-4b00-af66-7ac61d752ab7&'
            'currency=AUD&current_tab_id=home_tab&experiences_per_grid=20&fetch_filters=true&'
            'guidebooks_per_grid=20&has_zero_guest_treatment=true&hide_dates_and_guests_filters=true'
            '&is_guided_search=true&is_new_cards_experiment=true&is_standard_search=true&'
            'items_offset=0&items_per_grid=18&key=d306zoyjsyarp7ifhu67rjxn52tv0t20&locale=en-AU'
            '&metadata_only=false&query={2}&query_understanding_enabled=true&refinement_paths%5B%5D=%2Fhomes'
            '&satori_parameters=ERIA&satori_version=1.1.9&screen_height=722&screen_size=medium&screen_width=810'
            '&selected_tab_id=home_tab&show_groupings=true&supports_for_you_v3=true&timezone_offset=660'
            '&version=1.6.2&items_offset={0}&section_offset={1}')
            new_url = new_url.format(items_offset, section_offset, self.city)
    ...
    def parse_details(self, response):
        ...
        listing['picture_urls'] = response.meta['picture_urls']
