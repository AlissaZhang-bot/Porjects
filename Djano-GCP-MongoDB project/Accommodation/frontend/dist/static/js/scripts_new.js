...
function initCitybook() {
    ...
    $('input[name="bookdates"]').daterangepicker({
        ...
        isInvalidDate: function(date) {
            var occupied_ranges = $('#occupied_ranges').val();
            console.log(occupied_ranges);
            occupied_ranges = JSON.parse(occupied_ranges);
            var len = occupied_ranges.length;
            for (var i=0;i<len;i++){
                var startTime = new Array();
                startTime = occupied_ranges[i][0].split('-');
                var rangeStartTime = Date.UTC(parseInt(startTime[0]), parseInt(startTime[1]), parseInt(startTime[2]));
                var stopTime = new Array();
                stopTime = occupied_ranges[i][1].split('-');
                var rangeStopTime = Date.UTC(parseInt(stopTime[0]), parseInt(stopTime[1]), parseInt(stopTime[2]));
                var undifineDay = new Array();
                undifineDay = date.format('YYYY-M-D').split('-');
                var undifinedDate = Date.UTC(parseInt(undifineDay[0]), parseInt(undifineDay[1]), parseInt(undifineDay[2]));
                if (undifinedDate >= rangeStartTime && undifinedDate <= rangeStopTime){
                    return true;
                }
            };
        },
...
// formalize input bankcard number
$("#payment_card").on("keyup", formatBC);
function formatBC(e){
